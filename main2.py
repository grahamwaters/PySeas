...
# create a log file to store the output of the program
# if the log file already exists, then delete it and create a new one
...
save_confidence_plots = True # if True, will save the confidence plots
...
def check_buoy_status(buoy_id):
    ...
    try:
        ...
        if white_pixels / total_pixels > 0.9:
            ...
            # save the image with color (convert back to RGB)
            img = img.convert('RGB')
            ...
            # use a random uuid to make sure that the file name is unique
            # uuid = uuid4()
...
def main():
    ...
    try:
        ...
        if not os.path.exists('panels'):
            os.makedirs('panels')
        if not os.path.exists('BuoyEyeShots'):
            os.makedirs('BuoyEyeShots')
