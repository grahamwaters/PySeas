...
def make_panorama(self, images):
    ...
    if status == 0:
        # Resize stitched image to panorama size
        width = int(stitched.shape[1] * scale_percent / 100)
        height = int(stitched.shape[0] * scale_percent / 100)
        dim = (width, height)
        stitched = cv2.resize(stitched, dim, interpolation=cv2.INTER_AREA)
        return stitched
    else:
        return None
...
def artist_eval(image_path):
    ...
    def get_panel_center_position(panel_num):
        return (int(width * (panel_num * 2 - 1) / 12), int(height / 2))

    panel_centers = [img.getpixel(get_panel_center_position(i)) for i in range(1, 7)]

    mse_values = [np.mean((np.array(panel_centers[i]) - np.array(panel_centers[i + 1])) ** 2) for i in range(5)]
...
def unusual_plotting_tf_squeeze():
    start = time.time()
